...
namespace Faoma4.Controllers
{
    ...
    {
        ServerAccountService sAs = new ServerAccountService();
        ...
        {
            List<serverAccount> list = sAs.ListOfServerAccounts().ToList();
            return View(list);
        }
        ...
        public ActionResult Details(long? id)
        {
            ...
            serverAccount serverAccount = sAs.FindServerAccountWithId(id);
        ...
        public ActionResult Create([Bind(Include = "teBeherenEmail,teBeherenEmailPW,looptijd,beheerdersEmail")] serverAccount serverAccount)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                sAs.CreateServerAccount(serverAccount);

                sAs.TestLoginTeBeherenAccount(serverAccount);
        ...
        public ActionResult Edit(long? id,[Bind(Include = "teBeherenEmail,teBeherenEmailPW,looptijd,beheerdersEmail")] serverAccount serverAccount)
        {
            if (ModelState.IsValid)
            {
                sAs.EditServerAccount(serverAccount, id);
                sAs.SendEditConfirmaiton(serverAccount);
